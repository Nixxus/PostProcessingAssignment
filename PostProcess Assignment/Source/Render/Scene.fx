...
float4 PPTintShader( VS_BASIC_OUTPUT vOut ) : SV_Target
{
	...
	float2 UVScene;
	UVScene.x = vOut.ProjPos.x / ViewportWidth;
	UVScene.y = vOut.ProjPos.y / ViewportHeight;
...
float4 PPCutGlassShader( VS_NORMALMAP_OUTPUT vOut ) : SV_Target
{
	...
	float2 sceneUV;
	sceneUV.x = vOut.ProjPos.x / ViewportWidth;
	sceneUV.y = vOut.ProjPos.y / ViewportHeight;
